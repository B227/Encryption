...
void KeyExpansion(unsigned char* inputKey, unsigned char* expandedKeys)
{
  ...
  Serial.print("\n byte generated %");
  Serial.println(bytesGenerated);
    for (int i = 0; i < 4; i++)
    {
      int test = expandedKeys[bytesGenerated - 16] ^ temp[i];
      Serial.println("\n Tester");
      printHex(test);
      expandedKeys[bytesGenerated] = test;
      bytesGenerated++;
    }
  }
  for(int i = 16; i < 176; i++){
    printHex(expandedKeys[i]);
    }
}

void AddRoundKey(unsigned char* state, unsigned char* RoundKey)
{
  // Serial.print( "Roundkey"<<endl;
  for (int i = 0; i < 16; i++)
    printHex(RoundKey[i]);
  Serial.print( "Addkey \n");
  for (int i = 0; i < 16; i++)
